...
class GcsToBqTest(unittest.TestCase):
    ...
    def testStandardizeFrameOneDim(self):
        ...
        df = gcs_to_bq_util.values_json_to_df(json.dumps(self._fake_race_data))
        df = census.standardize_frame(df, group_vars, ["race"], False, "population")
        expected_df = gcs_to_bq_util.values_json_to_df(json.dumps(self._expected_race_data)).reset_index(drop=True)
    ...
    def testStandardizeFrameTwoDims(self):
        ...
        df = gcs_to_bq_util.values_json_to_df(json.dumps(self._fake_sex_by_age_data))
        df = census.standardize_frame(df, group_vars, ["sex", "age"], False, "population")
        expected_df = gcs_to_bq_util.values_json_to_df(json.dumps(self._expected_sex_by_age_data)).reset_index(
            drop=True
        )
        assert_frame_equal(expected_df, df)
