...
_expected_time_series_merged_with_pop_numbers = [
    ['time_period', 'state_fips', 'race_category_id', 'population', 'population_pct', 'other_col'],
...
def _get_pop_data_as_df(*args):
    pop_dtype = {std_col.STATE_FIPS_COL: str, std_col.POPULATION_COL: float, std_col.POPULATION_PCT_COL: float}
...
@mock.patch('ingestion.gcs_to_bq_util.load_public_dataset_from_bigquery_as_df', return_value=_get_fips_codes_as_df())
...
def testMergePopNumbersState(mock_pop: mock.MagicMock):
    ...
    expected_df = gcs_to_bq_util.values_json_to_df(
        json.dumps(_expected_merged_with_pop_numbers), dtype={std_col.STATE_FIPS_COL: str}
    ).reset_index(drop=True)
...
def testMergePopNumbersCounty(mock_pop: mock.MagicMock):
    df = gcs_to_bq_util.values_json_to_df(
        json.dumps(_data_without_pop_numbers_county), dtype={std_col.STATE_FIPS_COL: str, std_col.COUNTY_FIPS_COL: str}
    ).reset_index(drop=True)
...
def testMergeMultiplePopCols(mock_pop: mock.MagicMock):
    df = gcs_to_bq_util.values_json_to_df(
        json.dumps(_data_without_pop_numbers_multiple_rows), dtype={std_col.STATE_FIPS_COL: str}
    ).reset_index(drop=True)

    expected_df = gcs_to_bq_util.values_json_to_df(
        json.dumps(_expected_merge_with_pop_numbers_multiple_rows), dtype={std_col.STATE_FIPS_COL: str}
    ).reset_index(drop=True)
