...
def _load_public_dataset_from_bigquery_as_df(*args, **kwargs):
    ...
    if args[1] == "fips_codes_states":
        return gcs_to_bq_util.values_json_to_df(json.dumps(_public_state_dataset_from_bq), dtype=str).reset_index(
            drop=True
        )

    # within merge_county_names()
    if args[1] == "fips_codes_all":
        return gcs_to_bq_util.values_json_to_df(json.dumps(_public_county_dataset_from_bq), dtype=str).reset_index(
            drop=True
        )
