...
from ingestion.constants import US_FIPS, NATIONAL_LEVEL, STATE_LEVEL, COUNTY_LEVEL, RACE, AGE, SEX

from ingestion.standardized_columns import Race, add_race_columns_from_category_id, generate_column_name
                ...
                for county_level in [True, False]:
                    ...
                    file_diff = (
                        url_file_to_gcs.url_file_to_gcs(
                            self.base_url, params, bucket, self.get_filename_race(measure, race, county_level, year)
                        )
                        or file_diff
                    )
    ...
    def get_raw_data(self, demo, geo, metadata, gcs_bucket):
        ...
        if demo == RACE:
            ...
            for measure, acs_item in ACS_ITEMS.items():
                ...
                concept_df = self.generate_df_for_concept(
                    measure, acs_item, concept_df, demo, geo, acs_item.sex_age_concept, var_map
                )
